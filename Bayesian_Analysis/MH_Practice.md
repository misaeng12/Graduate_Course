실습:  예제 4.3에 대한 MH 알고리즘에서 theta의 후보표본을 다변량 정규분포에서 생성할 때 theta의 원소 모두에 같은 보폭(표준편차)을 사용하였다. 그런데 theta의 원소마다 분산이 다를 수 있고 따라서 적절한 보폭이 다를 수 있으므로 각 원소의 표준편차가 다른 다변량 정규분포를 고려하여 보자.
